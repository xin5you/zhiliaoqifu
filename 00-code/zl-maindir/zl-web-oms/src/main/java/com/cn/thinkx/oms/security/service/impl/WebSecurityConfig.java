package com.cn.thinkx.oms.security.service.impl;
...
import com.cn.thinkx.oms.security.controller.MyUsernamePasswordAuthenticationFilter;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public MyUsernamePasswordAuthenticationFilter myUsernamePasswordAuthenticationFilter() throws Exception {
		...
		usernamePasswordAuthenticationFilter.setRequiresAuthenticationRequestMatcher(new AntPathRequestMatcher("/user/doLogin.do", "POST"));
		// 登录失败之后返回的
	...
	public AccessDeniedHandler accessDeniedHandler() {
		...
		accessDeniedHandler.setErrorPage("/denied.jsp");
	...
	public SavedRequestAwareAuthenticationSuccessHandler loginLogAuthenticationSuccessHandler() {
		...
		loginLogAuthenticationSuccessHandler.setDefaultTargetUrl("/main/index.do");
	...
	public SimpleUrlAuthenticationFailureHandler simpleUrlAuthenticationFailureHandler() {
		// 请求跳转到登录页面
		return new SimpleUrlAuthenticationFailureHandler("/login/loginIndex.do");
	}
	...
	private SessionInformationExpiredStrategy sessionInformationExpiredStrategy() {
		return new SimpleRedirectSessionInformationExpiredStrategy("/login/loginIndex.do");
	}
	...
	private LoginUrlAuthenticationEntryPoint myAuthenticationEntryPoint() {
		// 未登录用户指定登录路径
		LoginUrlAuthenticationEntryPoint myAuthenticationEntryPoint = new LoginUrlAuthenticationEntryPoint("/login/loginIndex.do");
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/authcode/genAuthCode.do", "/login/loginIndex.do", "/login/main.do", "/login/doLogin.do", "/login/logout.do", "/main/error.do", "/denied.jsp").permitAll();
		http.exceptionHandling().accessDeniedPage("/denied.jsp");
		// 登录页面的访问地址，failureUrl("/login")：登录失败后转向的页面
		//defaultSuccessUrl("/main")：登录成功后转向的页面，invalidateHttpSession(true)： 注销行为任意访问
		http.authorizeRequests().anyRequest().authenticated().and().formLogin().loginPage("/login/login.jsp").failureUrl("/login/loginIndex.do")
				.permitAll().defaultSuccessUrl("/main/index.do").and().logout().permitAll().invalidateHttpSession(true);
		// 自定义登录
...
//		http.addFilterAfter(new ConcurrentSessionFilter(sessionRegistry, sessionInformationExpiredStrategy()), ConcurrentSessionFilter.class);
		http.exceptionHandling().authenticationEntryPoint(new LoginUrlAuthenticationEntryPoint("/login/loginIndex.do")).and().logout()
				.logoutUrl("/login/logout.do").logoutSuccessUrl("/login/loginIndex.do").and().exceptionHandling().accessDeniedPage("/main/error.do");
		//重置密码与退出登录时进行处理
		http.logout().logoutUrl("/login/logout.do").logoutSuccessUrl("/login/loginIndex.do").invalidateHttpSession(true);
		// session管理
		// session失效后跳转
		http.sessionManagement().invalidSessionUrl("/login/loginIndex.do");
		http.sessionManagement().maximumSessions(1).expiredUrl("/login/loginIndex.do");
		// 关闭csrf
		http.csrf().disable();
	}
