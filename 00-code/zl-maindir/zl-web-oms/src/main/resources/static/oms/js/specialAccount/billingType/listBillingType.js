...
var listBillingType = {
	...
	intoAddBillingType:function(){
		var url = Helper.getRootPath()+"/specialAccount/billingType/intoAddBillingType.do";
		location.href=url;
	},
	intoEditBillingType:function(){
		var bId = $(this).attr('bId');
		var url = Helper.getRootPath()+"/specialAccount/billingType/intoEditBillingType.do?bId="+bId;
		location.href=url;
	},
	...
	deleteBillingType:function(){
		...
		Helper.confirm("确定删除该商品？",function(){
			$.ajax({								  
				url: Helper.getRootPath() + '/specialAccount/billingType/deleteBillingTypeCommit.do',
				type: 'post',
				dataType : "json",
				data: {
					"bId": bId
