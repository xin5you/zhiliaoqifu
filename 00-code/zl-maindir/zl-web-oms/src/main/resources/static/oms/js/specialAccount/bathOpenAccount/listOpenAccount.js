...
var listOpenAccount = {
	...
	searchReset:function(){
		location = Helper.getRootPath() + '/speaccount/batch/listOpenAccount.do';
	},
	intoAddOpenAccount:function(){
		var url = Helper.getRootPath()+"/speaccount/batch/intoAddOpenAccount.do";
		location.href=url;
	},
	intoEditOpenAccount:function(){
		var orderId = $(this).attr('orderId');
		var url = Helper.getRootPath()+"/speaccount/batch/intoEditOpenAccount.do?orderId="+orderId;
		location.href=url;
	},
	intoViewOpenAccount:function(){
		var orderId = $(this).attr('orderId');
		var url = Helper.getRootPath()+"/speaccount/batch/intoViewOpenAccount.do?orderId="+orderId;
		location.href=url;
	},
	deleteOpenAccountCommit:function(){
		...
		Helper.confirm("您是否删除该记录？",function(){
		    $.ajax({								  
	            url: Helper.getRootPath() + '/speaccount/batch/deleteOpenAccountCommit.do',
	            type: 'post',
	            dataType : "json",
	            data: {
	                "orderId": orderId
	            ...
	            success: function (result) {
	            	if(result.status){
	            		location.href=Helper.getRootPath() + '/speaccount/batch/listOpenAccount.do?operStatus=4';
	            	}else{
	...
	addOrderCommit:function(){
		...
		Helper.confirm("您确认提交该订单吗？",function(){
		    $.ajax({								  
	            url: Helper.getRootPath() + '/speaccount/batch/addOrderCommit.do',
	            type: 'post',
	            dataType : "json",
	            data: {
	                "orderId": orderId
	...
	addOrderAgainCommit:function(){
		...
		Helper.confirm("您确认提交该订单吗？",function(){
		    $.ajax({								  
	            url: Helper.getRootPath() + '/speaccount/batch/addOrderAgainCommit.do',
	            type: 'post',
	            dataType : "json",
	            data: {
	                "orderId": orderId
