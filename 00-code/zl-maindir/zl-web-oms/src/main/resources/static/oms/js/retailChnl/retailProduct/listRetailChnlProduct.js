...
//		$('.btn-grant-role').on('click', listTelChannelProduct.intoAddChannelItemList);
	...
	searchReset: function(){
		location = Helper.getRootPath() + '/retailChnl/retailChnlProduct/listRetailChnlProduct.do';
	},
	intoAddTelChannelProduct:function(){
		var url = Helper.getRootPath()+"/retailChnl/retailChnlProduct/intoAddRetailChnlProduct.do";
		location.href=url;
	},
	intoEditTelChannelProduct:function(){
		var productId = $(this).attr('productId');
		var url = Helper.getRootPath()+"/retailChnl/retailChnlProduct/intoEditRetailChnlProduct.do?productId="+productId;
		location.href=url;
	},
	intoViewTelChannelProduct:function(){
		var productId = $(this).attr('productId');
		var url = Helper.getRootPath()+"/retailChnl/retailChnlProduct/intoViewRetailChnlProduct.do?productId="+productId;
		location.href=url;
	},
...
//	},
	deleteTelChannelProductCommit:function(){
		...
		Helper.confirm("您是否删除该记录？",function(){
		    $.ajax({								  
	            url: Helper.getRootPath() + '/retailChnl/retailChnlProduct/deleteRetailChnlProductCommit.do',
	            type: 'post',
	            dataType : "json",
	            data: {
	                "productId": productId
	            ...
	            success: function (result) {
	            	if(result.status){
	            		location.href=Helper.getRootPath() + '/retailChnl/retailChnlProduct/listRetailChnlProduct.do?operStatus=4';
	            	}else{
