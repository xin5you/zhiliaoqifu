...
var editOpenAccount = {
	...
	addOrderCommit:function(){
		...
		$.ajax({
			url: Helper.getRootPath() + '/speaccount/batch/addOrderListCommit.do',
            type: 'post',
            dataType : "json",
            data: {
            	"orderId":orderId,
            ...
            success: function (result) {
            	if(result.status) {
                	location = Helper.getRootPath() + '/speaccount/batch/intoEditOpenAccount.do?operStatus=1&orderId='+orderId;
                } else {
	...
	deleteOrderCommit:function(){
		...
		Helper.confirm("您确认删除该用户信息？",function(){
		    $.ajax({								  
	            url: Helper.getRootPath() + '/speaccount/batch/deleteOrderListCommit.do',
	            type: 'post',
	            dataType : "json",
	            data: {
	            ...
	            success: function (result) {
	            	if(result.status){
	            		location.href=Helper.getRootPath() + '/speaccount/batch/intoEditOpenAccount.do?operStatus=4&orderId='+orderId;
	            	}else{
