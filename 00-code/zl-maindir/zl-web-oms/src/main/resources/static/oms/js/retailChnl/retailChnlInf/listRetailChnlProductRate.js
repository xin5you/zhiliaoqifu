...
var listTelChannelProductRate = {
	...
	addTelChannelProductRates : function() {
		...
		$.ajax({
			url : Helper.getRootPath() + '/retailChnl/retailChnlInf/addRetailChnlRateCommit.do',
			type : 'post',
			dataType : "json",
			data : {
				channelId : channelId,
			...
			success : function(data) {
				...
				if (data.status) {
					location = Helper.getRootPath() + '/retailChnl/retailChnlInf/intoAddRetailChnlRate.do?channelId='+channelId;
				} else {
	...
	intoEditTelChannelProductRates:function(){
		var id = $(this).attr('id');
		var url = Helper.getRootPath()+"/retailChnl/retailChnlInf/intoEditRetailChnlProductRate.do?id="+id;
		location.href=url;
	}
}
