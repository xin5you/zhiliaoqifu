...
var addOpenAccount = {
	...
	accountImport:function(){
		...
		$("#uploadMainForm").ajaxSubmit({
			...
			success: function(data){
				if(data.status) {
                	location.href=Helper.getRootPath() + '/batch/openAccount/intoAddOpenAccount.do';
                }else{
	...
	addAccountInf:function(){
		...
		$.ajax({
            ...
            success: function (result) {
            	if(result.status) {
                	location = Helper.getRootPath() + '/batch/openAccount/intoAddOpenAccount.do';
                } else {
	...
	deleteAccountInf:function(){
		...
		Helper.confirm("您确定删除该用户信息？",function(){
			$.ajax({								  
				url: Helper.getRootPath() + '/batch/openAccount/deleteAccountInf.do',
				type: 'post',
				dataType : "json",
				data: {
					"puId": puId
