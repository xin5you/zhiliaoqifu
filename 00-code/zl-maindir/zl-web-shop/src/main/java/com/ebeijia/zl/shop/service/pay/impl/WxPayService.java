...
//        Order order = orderRepository.findByPrepayId(success.getMerchantOutOrderNo());
//        if (order==null){
...
//                param.put("mch_id", order.getMchtId());
//                url = getUrlParamsByMap(order.getSuccessUrl(), param);
...
//            param.put("mchtId", order.getMchtId());
...
//        Order order = orderRepository.findByPrepayId(back.getMerchantOutOrderNo());
//        if (order == null) {
//            throw new BizException(ResultState.NOT_FOUND, "");
//        }
//
//        //效验参数
//        WxPayBackMsgDTO msgDTO = checkData(order.getMchtId(), back);
//
//        //没有第四方订单号就是我们自己平台的
//        if (order.getOutTradeNo() != null && order.getOutTradeNo().length() > 0) {
//
...
//     */
//    public String applyPay(WxPayReqDTO payInfo, MchtSett mchtSett) {
//        //生成系统订单
...
//        }
//        //申请支付
//        EBJPushOrder epo = new EBJPushOrder.Builder(mchtSett.getMerId(), mchtSett.getMerKey())
//                .merchantOutOrderNo(order.getPrepayId())
//                .noncestr(PayConstant.noncestr)
//                .orderMoney(String.valueOf(order.getAmt()))
